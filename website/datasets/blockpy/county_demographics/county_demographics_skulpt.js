...
var $builtinmodule = function(name)
{
    ...
    mod.get = new Sk.builtin.func(function(property, index, index_value) {
        ...
        var dataset = blockpy._IMPORTED_DATASETS["county_demographics"];
        var data = [];
        if (index != '(None)') {
    ...
    mod.get_report = new Sk.builtin.func(function() {
        ...
        if (!("county_demographics" in blockpy._IMPORTED_COMPLETE_DATASETS)) {
            alert("This library has not finished loading yet. Please wait about 10 seconds and try again.")
        } else {
            return Sk.ffi.remapToPy(blockpy._IMPORTED_COMPLETE_DATASETS["county_demographics"]);
        }
    });
